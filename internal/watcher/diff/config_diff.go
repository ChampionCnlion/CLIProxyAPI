...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if oldCfg.RemoteManagement.DisableAuthBan != newCfg.RemoteManagement.DisableAuthBan {
		changes = append(changes, fmt.Sprintf("remote-management.disable-auth-ban: %t -> %t", oldCfg.RemoteManagement.DisableAuthBan, newCfg.RemoteManagement.DisableAuthBan))
	}
